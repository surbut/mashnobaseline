...
Now we're going to simulate data across 20 subgroups according to 5 factors:
...
c=chat_sim_fsimple(n=10000,d=20,betasd=1,esd=0.1,K=10)
...
```{r}
saveRDS(c,"chatsimesparseftwenty.rds")
...
write.table(sparsestrongt,"sparsestrong_twenty.txt",col.names = FALSE,row.names=FALSE)
...
```{r}
system('/Users/sarahurbut/miniconda3/bin/sfa -gen sparsestrong_twenty.txt -g 557 -k 5 -n 20 i -o sparsestrong_twenty')
A="sparsestrongtwenty"

factor.mat=as.matrix(read.table("sparsestrong_twenty_F.out"))
lambda.mat=as.matrix(read.table("sparsestrong_twenty_lambda.out"))
...
```{r}
mash.means=as.matrix(read.table("sparseLtwentyposterior.means.txt")[,-1])
lfsr.mash=as.matrix(read.table("sparseLtwentylfsr.txt"))[,-1]
edcov=readRDS("covmatsparseLtwenty.rds")
j=10
...
pis=readRDS("pissparseLtwenty.rds")$pihat
...
```{r}
rm(list=ls())
c=readRDS("chatsimesparseftwenty.rds")
...
covmash=compute.hm.covmat.all.max.step(max.step = max.step,b.hat = wfull,se.hat = se,t.stat = strongprojectedtsimulations,Q = 5,lambda.mat = lambda.mat,A = "sparseoldmashtwenty",factor.mat = factor.mat,zero = T,power = 2)$covmat
...
```{r}
mash.means=as.matrix(read.table("sparseoldmashtwentyposterior.means.txt")[,-1])
lfsr.mash=as.matrix(read.table("sparseoldmashtwentylfsr.txt"))[,-1]
cov=readRDS("covmatsparseoldmashtwenty.rds")
pis=readRDS("pissparseoldmashtwenty.rds")$pihat
w=data.frame(wfull)
R=ncol(w)
j=10
...
mash.nobaselinemeans=as.matrix(read.table("sparseLtwentyposterior.means.txt")[,-1])
mash.means=as.matrix(read.table("sparseoldmashtwentyposterior.means.txt")[,-1])
...
barplot(c$factors[10,])
j=which(c$components==10)[3]


par(mfrow=c(2,2))
barplot(c$beta[j,])
barplot(c$chat[j,]-mean(c$chat[j,]),ylim=c(-1,1))
barplot(mash.nobaselinemeans[j,],ylim=c(-1,1))
barplot(mash.means[j,],ylim=c(-1,1))
```
...
lfsr.mash=as.matrix(read.table("sparseoldmashtwentylfsr.txt"))[,-1]
lfsr.nobase=as.matrix(read.table("sparseLtwentylfsr.txt"))[,-1]
