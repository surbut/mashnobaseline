...
Now we're going to simulate data across 40 subgroups according to 5 factors:
...
c=chat_sim_fsimple(n=10000,d=40,betasd=1,esd=0.1,K=10)
...
```{r}
saveRDS(c,"chatsimesparsefforty.rds")
...
write.table(sparsestrongt,"sparsestrong_forty.txt",col.names = FALSE,row.names=FALSE)
...
```{r}
system('/Users/sarahurbut/miniconda3/bin/sfa -gen sparsestrong_forty.txt -g 557 -k 5 -n 40 i -o sparsestrong_forty')
A="sparsestrongforty"

factor.mat=as.matrix(read.table("sparsestrong_forty_F.out"))
lambda.mat=as.matrix(read.table("sparsestrong_forty_lambda.out"))
...
```{r}
mash.means=as.matrix(read.table("sparseLfortyposterior.means.txt")[,-1])
lfsr.mash=as.matrix(read.table("sparseLfortylfsr.txt"))[,-1]
edcov=readRDS("covmatsparseLforty.rds")
j=10
...
pis=readRDS("pissparseLforty.rds")$pihat
...
```{r}
rm(list=ls())
c=readRDS("chatsimesparsefforty.rds")
...
covmash=compute.hm.covmat.all.max.step(max.step = max.step,b.hat = wfull,se.hat = se,t.stat = strongprojectedtsimulations,Q = 5,lambda.mat = lambda.mat,A = "sparseoldmashforty",factor.mat = factor.mat,zero = T,power = 2)$covmat
...
```{r}
mash.means=as.matrix(read.table("sparseoldmashfortyposterior.means.txt")[,-1])
lfsr.mash=as.matrix(read.table("sparseoldmashfortylfsr.txt"))[,-1]
cov=readRDS("covmatsparseoldmashforty.rds")
pis=readRDS("pissparseoldmashforty.rds")$pihat
w=data.frame(wfull)
R=ncol(w)
j=10
...
mash.nobaselinemeans=as.matrix(read.table("sparseLfortyposterior.means.txt")[,-1])
mash.means=as.matrix(read.table("sparseoldmashfortyposterior.means.txt")[,-1])
...
lfsr.mash=as.matrix(read.table("sparseoldmashfortylfsr.txt"))[,-1]
lfsr.nobase=as.matrix(read.table("sparseLfortylfsr.txt"))[,-1]
