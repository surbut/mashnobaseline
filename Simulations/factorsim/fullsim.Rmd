...
##choose the strong projected T##
library("ExtremeDeconvolution")
strongprojectedtsimulations=t(L%*%t(t[index,]))
max.step=deconvolution.em.with.bovy(t.stat = strongprojectedtsimulations,factor.mat = factor.mat,v.j = s.j,lambda.mat = lambda.mat,K = 3,3)

##Show how the ranks is preserved
apply(max.step$true.covs,1,function(x){qr(x)$rank})


covmash=compute.hm.covmat.all.max.step(max.step = max.step,b.hat = wfull,se.hat = se,t.stat = strongprojectedtsimulations,Q = 5,lambda.mat = lambda.mat,A = "oldmash",factor.mat = factor.mat,zero = T,power = 2)$covmat
