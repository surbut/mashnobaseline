...
\item When denoising with Bovy, our previous approach used the matrix of maximum t statistics to both initialize and train the BovyEM. Now, we will initalize with the MXR matrix of $L_{R,R}T$ and train the EM on the MxR-1 matrix of maximum $L_{R,R-1} T$
\item When choosing omega, we will use the diagonal of $L V L'$, where $V$ is $D(s.j^2)$
\end{itemize}


\section{Likelihood}

Now we will replace the RxR matrix $L$ with the RxR-1 matrix $L*$, effectively removing a data point from the observed uncentered statistics, such that the rank of the marginal variance of $w$ is guaranteed to be equal to the dimension of $w$.
...
%\item In practice, it probably won't matter if we use the $L_{R-1,R} $ or $L_{R,R}$ to choose the scaling factors, but it becomes a bit tricky when we specify the standard errors as the diagonal of $L_{R-1,R} V L_{R-1,R} '$, since we know this marginal covariance is not really diagonal.
%\end{itemize}
