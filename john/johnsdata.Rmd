...
```{r,echo=F,eval=FALSE}
d=read.table("~/Dropbox/john/s1table.txt",header = T)
genenames=d[,1]
dataframe=d[,-c(1,2)]
rownames(dataframe)=genenames
annotation=read.table("~/Dropbox/john/annotation.txt",header = T)
```



```{r,echo=F,eval=T}
# groupbytime.eighteen=grep(".18",colnames(dataframe))
# groupbytime.fortyeight=grep(".48",colnames(dataframe))
# lattertimes=union(groupbytime.eighteen,groupbytime.fortyeight)
# d=seq(1:ncol(dataframe))
# groupbytime.four=d[-which(d%in%lattertimes)]
...
Now fit the model for all genes to each level of the 26 subgroups iteratively. Essentially, performing univariate regression for all genes on each time*bacterial combination:
...
write.table(beta,"betafit.txt",col.names = T)
write.table(se,"sefit.txt",col.names = T)
write.table(t,"tfit.txt",col.names = T)

##check to make sure

lm(voom_class_adj[,37]~m[,9]-1)##model the expression of gene 37 on factor 9
t[37,9]
##show that beta is equal to the mean for those individuals

mean(voom_class_adj[which(m[,9]==1),37])##mean gene expression for time point 18 individuals at gene 37
beta[37,9]
```
...
```{r,echo=TRUE,eval=F}
t=read.table("tfit.txt")
b=read.table("betafit.txt")
se=read.table("sefit.txt")
...
write.table(t[index,],"maxt.txt",col.names = F ,row.names = F)


system('/Users/sarahurbut/miniconda3/bin/sfa -gen maxt.txt -g 12538 -k 5 -n 26 i -o john')

A="john"

factor.mat=as.matrix(read.table("john_F.out"))
lambda.mat=as.matrix(read.table("john_lambda.out"))

library('mash')
cov=compute.covmat(b.gp.hat = t,sebetahat = s.j,Q =5, t.stat=t[index,],lambda.mat=lambda.mat,P=3,A=A, factor.mat = factor.mat,bma = TRUE,zero=TRUE,power=2)$covmat

omega=compute.covmat(b.gp.hat = t,sebetahat = s.j,Q =5, t.stat=t[index,],lambda.mat=lambda.mat,P=3,A=A, factor.mat = factor.mat,bma = TRUE,zero=TRUE,power=2)$omega
...
library('mash')
t=read.table("tfit.txt")
lf.ash=read.table('lfash.txt')
cov=readRDS("covmatjohn.rds")
library('gplots')
library('colorRamps')

for(k in c(2,3,9)){
  x=cov[[k]]/max(diag(cov[[k]]))
colnames(x)=rownames(x)=as.matrix(levels(ID)[-24])[,1]
  heatmap.2(x,revC = T,col = blue2green(256),dendrogram="none",density="none",trace="none")
v=svd(cov[[k]])$v
barplot(v[,1]/max(v[,1]),names=as.matrix(levels(ID)[-24])[,1],las=2)
}
lfsr=read.table("johnlfsr.txt")[,-1]
postt=read.table("johnposterior.means.txt")[,-1]
mean(lfsr<0.05)
mean(lf.ash<0.05)
