...
Let's try this out with john's data at time point 18, and notperform the centering in advnace. Critcally, I'm going to use the raw counts, and then perform the voom normalization myself, and use the weights from limma.
...
###here will be the ws and lvls, these need to be R-1xR-1
L=L[-1,]
w=t(L%*%t(t))
wmax=w[index,]
dim(wmax)
...
##and here are the initalizing t.stats,e.g., the MxR matrix of maximums acorss all
L=diag(R)-1/R*as.vector(rep(1,R))%*%t(as.vector(rep(1,R)))
t.stat=(t(L%*%t(t[index,])))
dim(t.stat)
```
Curretnly, Bovy's R apprroach requires the S be diagonal (i.e., you put in a matrix of standard erros) when we know that w|Lv ~ N(Lv,LVL'), so we use our old covariance function with the matrices that we would have used to initalize Bovy (i.e., various rank approximations of the empirical covariance matrix of centered T statistics).
...
sjmat=convertstandarderrors(s.j = s.j,L= L)##make this be the standard errors of all, it is the diagaonl of LVL' for each j.
dim(allcenteredt)
dim(sjmat)
...
A="jul22"
c=compute.covmat(b.gp.hat = allcenteredt,sebetahat = sjmat,Q = 5,t.stat = maxt,lambda.mat = lambda.mat,P = 3,A = "jul22",factor.mat = factor.mat,bma = T,zero = T,power = 2)$cov
...
compute.hm.train.log.lik.pen.with.L(w,se.train = s.j,covmat = c,A = A,pen = 1,L = L[-1,])

pis=readRDS(paste0("pis",A,".rds"))$pihat
cov=c

for(j in 1:nrow(w)){
  total.quant.per.snp.no.baseline(j = j,covmat = cov,b.gp.hat = w,se.gp.hat = s.j,pis = pis,A = A,checkpoint = F,L = L[-1,])}
...
for(k in 2:9){
  x=cov([[k]])
  colnames(x)=rownames(x)=colnames(t)
  heatmap.2(cov[[k]],revC=T,dendrogram=F,main=paste0("Pattern",k))}
}
```