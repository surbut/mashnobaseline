...
Let's try this out with john's data at time point 18, and not perform the centering in advnace. Critcally, I'm going to use the raw counts from John's Repo, and then perform the voom normalization myself, and use the weights from limma.
...
colSums(model_mat)[c(7:14)]
beta_class <- matrix(0, dim(counts_class)[1], 8);
sebeta_class <- matrix(0, dim(counts_class)[1], 8)
t_class = matrix(0, dim(counts_class)[1], 8)
...
dim(counts_class)
voom.obj=voom(y,design=model_mat)
...
for(k in c(1:8)){
  ...
  vsarah <- voom(y[,knownsamples], model_mat_temp)
  vsarah2 <- voom(y[,knownsamples], model_mat)
  vjohn <- voom(y[,knownsamples], design[knownsamples,])
  ...
  ##ask why not the same as l=lmFit(object = tables1[,knownsamples],model_mat_temp[,1],weights=vsarah$weights)
  beta_class[,k-6] <- as.matrix(limma.obj$coefficients[,1]);
  sebeta_class[,k-6] <- limma.obj$sigma*(as.matrix(limma.obj$stdev.unscaled[,1]));
  t_class[,k-6] = (as.matrix(limma.obj$coefficients[,1]))/(limma.obj$sigma*(as.matrix(limma.obj$stdev.unscaled[,7])));
  }
...
write.table(beta_class,"beta_voomuncenterednew.txt",col.names = T)
write.table(sebeta_class,"sebeta_voomuncenterednew.txt",col.names = T)
write.table(t_class,"tvoom_uncenterednew.txt",col.names = T)
```
...
j=8
k=3
lmFit(norm.expression,cbind(model_mat[,c((1:6),k+6)]),weights = weights)$coefficients[j,7]
beta_class[j,k]
##show that beta is close to the (unadjusted) mean for those individuals

mean(norm.expression[37,which(model_mat[,k+6]==1)])##mean gene expression for time point 18 individuals at gene 37
beta_class[37,3]
beta_class[37,3]/sebeta_class[37,3]
t_class[37,3]
```
...
```{r}
b=read.table("beta_voomuncenterednew.txt")
se=read.table("sebeta_voomuncenterednew.txt")
t=read.table("tvoom_uncenterednew.txt")
...
```{r,eval=TRUE}
index=which(rowSums(abs(tcenteredfull)>3)>0)
...
```{r,echo=F,eval=F}
write.table(strongprojectedt,"strongprojectedtnew.txt",col.names = F ,row.names = F)
```
...
```{r}
system('/Users/sarahurbut/miniconda3/bin/sfa -gen strongprojectedtnew.txt -g 7028 -k 5 -n 8 i -o jnbnew')
A="jnbnew"

factor.mat=as.matrix(read.table("jnbnew_F.out"))
lambda.mat=as.matrix(read.table("jnbnew_lambda.out"))
...
dim(sjmat)
A="jul25"
cov=compute.covmat(b.gp.hat = wfull,sebetahat = sjmat,Q = 5,t.stat = maxt,lambda.mat = lambda.mat,P = 3,A = "jul25",factor.mat = factor.mat,bma = T,zero = T,power = 2)$cov
...
mash.means=read.table("jul25posterior.means.txt")[,-1]
...
library('gplots')
library('colorRamps')
barplot(colSums(matrix(pis[-487],byrow = T,ncol=18)))
