...
w=t(L%*%t(t))
hist(w)
dim(w)##see that this omits a data point

##now we want to compute the lfsr on the centered, but this is a problem because LVL' is obviously not diagonal. So we choose the genes who have max observed deviation greater than 2 in at least one stugbroutp

# lf.ash=matrix(NA,ncol=ncol(w),nrow=nrow(w))
# 
# for(i in 1:ncol(w)){
#   lf.ash[,i]=ash(betahat = w[,i],sebetahat = swj[,i],mixcompdist = "normal")$lfsr
# }
...
write.table(lf.ash,"lfash18unadjusted.txt")
```
...
```{r}
system('/Users/sarahurbut/miniconda3/bin/sfa -gen strongprojectedt.txt -g 78 -k 5 -n 8 i -o johnnobaseline')
A="johnnobaseline"
